...
class TextCategorizer(TrainablePipe):
    ...
    def __init__(
        ...
        *,
        ...
        self.scorer = scorer
        if class_weights:
            self.class_weights = numpy.array(class_weights, dtype=numpy.float32)
        else:
    ...
    def update(
        ...
        *,
        ...
        if self.class_weights is not None:
            loss, d_scores = self.get_loss(examples, scores, self.class_weights)
        else:
    ...
    def get_loss(self, 
                 examples: Iterable[Example],
                 scores,
                 weights : Optional[numpy.ndarray] = None) -> Tuple[float, float]:
        ...
        weights = self.model.ops.asarray(weights)
        ...
        if weights is not None:
            squared = d_scores**2
            mean_square_error = numpy.sum(squared * weights) / numpy.sum(weights)
            d_scores *= weights
        else:
